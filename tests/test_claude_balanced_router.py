...
from claudex_gateway.claude_unified_headers import HeaderDescriptor
...
# ==========================================================================
# Unified-header ingestion: reset comparison, merge rules, warning
# retention/clearing, serving-account scoping, coalesced persistence (T-15).
#
# `claude_unified_headers.RECOGNIZED_HEADERS` is the committed, currently
# EMPTY wire table (no T-14 capture was available at commit time), so every
# merge rule below is proven fixture-independently against an INJECTED
# synthetic recognized-header table instead -- never against real captured
# header names.
# ==========================================================================

_SYNTHETIC_RECOGNIZED_HEADERS: dict[str, HeaderDescriptor] = {
    "x-synthetic-five-hour-used": HeaderDescriptor(window="five_hour", field="used_percent"),
    "x-synthetic-five-hour-status": HeaderDescriptor(window="five_hour", field="status"),
    "x-synthetic-five-hour-reset": HeaderDescriptor(window="five_hour", field="reset"),
    "x-synthetic-seven-day-used": HeaderDescriptor(window="seven_day", field="used_percent"),
    "x-synthetic-seven-day-reset": HeaderDescriptor(window="seven_day", field="reset"),
    "x-synthetic-fable-weekly-used": HeaderDescriptor(window="fable_weekly", field="used_percent"),
    "x-synthetic-fable-weekly-reset": HeaderDescriptor(window="fable_weekly", field="reset"),
}


def test_unified_reset_identity_bucketizes_within_the_thirty_second_tolerance() -> None:
    # Two epochs a mere 10s apart -- clock-conversion jitter, not a real
    # rollover -- compare equal.
    assert unified_reset_identity("five_hour", 1_000.0) == unified_reset_identity("five_hour", 1_010.0)
    # A different window is always a different identity, even at the exact same epoch.
    assert unified_reset_identity("five_hour", 1_000.0) != unified_reset_identity("seven_day", 1_000.0)
    # Far enough apart to land in a different 30s bucket: a different identity.
    assert unified_reset_identity("five_hour", 1_000.0) != unified_reset_identity("five_hour", 1_040.0)
    # No reset at all never claims an identity.
    assert unified_reset_identity("five_hour", None) is None


def test_unified_header_ingestion_merges_used_percent_and_coalesces_a_durable_write() -> None:
    store = _FakeStore()
    clock = _FakeClock()
    router = _make_router(store=store, clock=clock)

    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "42.5", "x-synthetic-five-hour-reset": "1700000030"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
        model="claude-sonnet-5",
        recognized=_SYNTHETIC_RECOGNIZED_HEADERS,
    )

    assert router.observations.window_pressure("acct-a", "five_hour", now=clock()) == 42.5
    assert len(store.usage_observation_calls) == 1
    call = store.usage_observation_calls[0]
    assert call["account_id"] == "acct-a"
    assert call["window"] == "five_hour"
    assert call["used_percent"] == 42.5
    assert call["unified_status"] is None
    assert call["unified_claim"] is not None


def test_unified_header_ingestion_same_identity_keeps_the_maximum_utilization() -> None:
    clock = _FakeClock()
    router = _make_router(store=_FakeStore(), clock=clock)
    reset = "x-synthetic-five-hour-reset"

    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "40", reset: "1700000030"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "30", reset: "1700000040"},  # same 30s bucket
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    assert router.observations.window_pressure("acct-a", "five_hour", now=clock()) == 40.0


def test_unified_header_ingestion_changed_identity_accepts_a_lower_value() -> None:
    clock = _FakeClock()
    router = _make_router(store=_FakeStore(), clock=clock)
    reset = "x-synthetic-five-hour-reset"

    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "90", reset: "1700000000"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    # A genuinely new reset boundary (>=30s away) legitimately lowers usage.
    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "10", reset: "1700000900"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    assert router.observations.window_pressure("acct-a", "five_hour", now=clock()) == 10.0


def test_unified_header_ingestion_drops_a_stale_observation_older_than_the_stored_one() -> None:
    clock = _FakeClock(1_000.0)
    router = _make_router(store=_FakeStore(), clock=clock)

    clock.value = 2_000.0
    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "60"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    # A reading whose own `now` predates the stored reading's `observed_at`
    # (an out-of-order, delayed arrival) never overwrites it.
    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "5"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    assert router.observations.window_pressure("acct-a", "five_hour", now=2_000.0) == 60.0


def test_unified_header_ingestion_malformed_used_percent_never_erases_existing_data() -> None:
    store = _FakeStore()
    clock = _FakeClock()
    router = _make_router(store=store, clock=clock)

    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "77"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    store.usage_observation_calls.clear()

    # A non-numeric value fails to parse -- a malformed, unusable reading --
    # and must never touch the existing 77.0.
    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "not-a-number"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    assert router.observations.window_pressure("acct-a", "five_hour", now=clock()) == 77.0
    assert store.usage_observation_calls == []


def test_unified_header_ingestion_incomplete_reset_never_lowers_the_stored_value() -> None:
    clock = _FakeClock()
    router = _make_router(store=_FakeStore(), clock=clock)

    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "80", "x-synthetic-five-hour-reset": "1700000000"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    # A usable percent but NO reset header at all -- incomplete, not malformed
    # -- must never lower the stored 80.0, even though 20 < 80.
    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "20"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    assert router.observations.window_pressure("acct-a", "five_hour", now=clock()) == 80.0


def test_unified_header_ingestion_never_fabricates_fable_weekly_for_a_non_fable_model() -> None:
    clock = _FakeClock()
    router = _make_router(store=_FakeStore(), clock=clock)

    router.ingest_unified_response_headers(
        {
            "x-synthetic-five-hour-used": "15",
            "x-synthetic-fable-weekly-used": "99",
            "x-synthetic-fable-weekly-reset": "1700000000",
        },
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
        model="claude-sonnet-5",  # default family, NOT fable
        recognized=_SYNTHETIC_RECOGNIZED_HEADERS,
    )

    assert router.observations.window_pressure("acct-a", "five_hour", now=clock()) == 15.0
    assert router.observations.window_pressure("acct-a", "fable_weekly", now=clock()) is None

    router.ingest_unified_response_headers(
        {"x-synthetic-fable-weekly-used": "99", "x-synthetic-fable-weekly-reset": "1700000000"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
        model="claude-fable-5",  # a genuine fable-family request DOES merge it
        recognized=_SYNTHETIC_RECOGNIZED_HEADERS,
    )
    assert router.observations.window_pressure("acct-a", "fable_weekly", now=clock()) == 99.0


def test_unified_header_ingestion_updates_only_the_serving_account() -> None:
    store = _FakeStore()
    clock = _FakeClock()
    router = _make_router(store=store, clock=clock)

    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "50"},
        account_id="acct-a",
        serving_account_id="acct-b",  # a different account than the one that responded
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
        model="claude-sonnet-5",
        recognized=_SYNTHETIC_RECOGNIZED_HEADERS,
    ...
    assert router.observations.window_pressure("acct-a", "five_hour", now=clock()) is None
    assert store.usage_observation_calls == []


def test_unified_header_ingestion_is_inert_with_the_committed_empty_recognized_table() -> None:
    store = _FakeStore()
    clock = _FakeClock()
    router = _make_router(store=store, clock=clock)

    # Uses the real, committed (currently empty) default table -- no `recognized=` override.
    router.ingest_unified_response_headers(
        {"anthropic-ratelimit-five-hour-used": "50"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
...
def test_unified_header_ingestion_allowed_warning_applies_a_haircut_and_expires_after_five_minutes() -> None:
    clock = _FakeClock(1_000.0)
    router = _make_router(store=_FakeStore(), clock=clock)
    reset = "x-synthetic-five-hour-reset"

    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "10", reset: "1700000000", "x-synthetic-five-hour-status": "allowed_warning"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    assert router.observations.has_active_warning("acct-a", "five_hour", now=clock.value + 60) is True
    assert router.observations.has_active_warning("acct-a", "five_hour", now=clock.value + 10 * 60) is False


def test_unified_header_ingestion_newer_valid_allowed_clears_the_warning() -> None:
    clock = _FakeClock(1_000.0)
    router = _make_router(store=_FakeStore(), clock=clock)
    reset = "x-synthetic-five-hour-reset"
    ...
    assert router.observations.has_active_warning("acct-a", "five_hour", now=clock.value) is True

    clock.value += 30.0  # well within the 5-minute freshness window
    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "12", reset: "1700000000", "x-synthetic-five-hour-status": "allowed"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    # cleared immediately -- never had to wait out its own 5-minute staleness.
    assert router.observations.has_active_warning("acct-a", "five_hour", now=clock.value) is False


def test_unified_header_ingestion_never_installs_a_cooldown_or_creates_migration_state() -> None:
    router = _make_router(store=_FakeStore())

    router.ingest_unified_response_headers(
        {"x-synthetic-five-hour-used": "99.9", "x-synthetic-five-hour-status": "allowed_warning"},
        account_id="acct-a",
        serving_account_id="acct-a",
        account_incarnation_id="inc-a",
        account_profile_fingerprint="fp-a",
    ...
    assert router.account_cooldown_deadline("acct-a") is None
    assert router.family_cooldown_deadline("acct-a", "default") is None
    assert router.active_migration_count() == 0


def test_claude_unified_headers_module_never_touches_the_filesystem_when_reloaded_and_parsed(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Step 7's production-layout guard: block `open`, `Path.read_text`, and
    package-resource access outright, then reload `claude_unified_headers`
    and parse with its own committed constants -- neither step may touch any
    of them.
    """

    def _blocked_open(*_args: Any, **_kwargs: Any) -> Any:
        raise AssertionError("claude_unified_headers must never call open() at import or parse time")

    def _blocked_read_text(*_args: Any, **_kwargs: Any) -> Any:
        raise AssertionError("claude_unified_headers must never call Path.read_text() at import or parse time")
    ...
    try:
        ...
        monkeypatch.setattr(importlib_resources, "files", _blocked_open, raising=False)
        monkeypatch.setattr(importlib_resources, "read_text", _blocked_open, raising=False)
    except ImportError:
        pass
    try:
        import pkgutil

        monkeypatch.setattr(pkgutil, "get_data", _blocked_open, raising=False)
    except ImportError:
        pass

    reloaded = importlib.reload(claude_unified_headers)
